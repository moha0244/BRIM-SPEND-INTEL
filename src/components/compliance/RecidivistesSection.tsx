...
export function RecidivistesSection({ violations }: { violations: Violation[] }) {
        ...
        <thead>
          <tr style={{ borderBottom: "1px solid #1a2236" }}>
            {["Carte", "Violations", "Montant total"].map((h, i) => (
              <th key={h} style={{ textAlign: i === 0 ? "left" : "right", padding: "8px 16px", fontSize: 9, color: "#2A3A55", fontWeight: 400, fontFamily: MONO, textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
